...
func TestSimpleTimeSeries(t *testing.T) {
	...
	assert.Equal(t, simpleTimeSeries.lastStats[0].Cnt(), int64(2))
	assert.Equal(t, simpleTimeSeries.lastStats[0].Sum(), 11.0)
	assert.Equal(t, simpleTimeSeries.lastStats[1].Cnt(), int64(2))
	assert.Equal(t, simpleTimeSeries.lastStats[1].Sum(), 13.0)
	...
	assertEqual(t, simpleTimeSeries.lastStats[0].Cnt(), int64(3))
	assertEqual(t, simpleTimeSeries.lastStats[0].Sum(), 21.0)
	assertEqual(t, simpleTimeSeries.lastStats[1].Sum(), 24.0)
	...
	assertEqual(t, simpleTimeSeries.lastStats[0].Cnt(), int64(3))
	assertEqual(t, simpleTimeSeries.lastStats[0].Sum(), 24.0)
	assertEqual(t, simpleTimeSeries.lastStats[1].Sum(), 27.0)
...
func TestTimeSeriesContainer(t *testing.T) {
	...
	for _, metricsTopic := range metricsTopicArr {
		...
		stats, descOfPodTimeSeries := container.GetStatisticsOfPod("test", metricsTopic)
		...
		assert.True(t, container.StatisticsOfPodHasPositiveDelta("test", metricsTopic))
		...
		assert.False(t, container.StatisticsOfPodHasPositiveDelta("test", metricsTopic))
